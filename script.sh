...
install_marzneshin() {
    # Fetch releases
    FILES_URL_PREFIX="https://raw.githubusercontent.com/jenaze/marzneshin/master"
	COMPOSE_FILES_URL="https://raw.githubusercontent.com/jenaze/marzneshin-deploy/master"
 	database=$1
  	nightly=$2
	...
	if [ "$nightly" = true ]; then
	    colorized_echo red "setting compose tag to nightly."
	 	sed -ri "s/(jenaze\/marzneshin:)latest/\1nightly/g" $CONFIG_DIR/docker-compose.yml
	fi
...
install_marznode_xray_config() {
    ...
    curl -sL "https://raw.githubusercontent.com/jenaze/marznode/master/xray_config.json" -o "$NODE_DATA_DIR/xray_config.json"
